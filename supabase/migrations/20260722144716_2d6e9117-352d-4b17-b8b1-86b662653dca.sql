...
ALTER TABLE public.gdpr_requests ADD COLUMN IF NOT EXISTS is_test boolean NOT NULL DEFAULT false;
CREATE INDEX IF NOT EXISTS idx_gdpr_requests_is_test ON public.gdpr_requests(is_test);

CREATE OR REPLACE FUNCTION public.submit_rights_request(_request_type text, _request_description text, _requester_contact text, _requester_name text, _representative_name text DEFAULT NULL::text, _channel text DEFAULT 'website'::text)
 RETURNS text
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF v_uid IS NULL THEN
    ...
    v_hash := extensions.digest(lower(trim(_requester_contact)), 'sha256');
    ...
    IF v_recent_same >= 3 THEN
      RAISE EXCEPTION 'You have reached the automated submission limit. You may still exercise your data-protection rights through the Trust''s secure contact form at /contact-the-trust.';
    END IF;
