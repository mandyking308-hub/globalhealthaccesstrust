...
DECLARE
  v_slugs text[] := ARRAY[
    'terms-of-use','privacy-notice','cookie-notice','project-team-terms',
    'complaints-policy','safeguarding-policy','media-policy'
  ];
  v_doc record;
  v_version_id uuid;
  v_approver uuid;
  v_found integer;
BEGIN
  SELECT count(*) INTO v_found FROM public.legal_documents WHERE slug = ANY(v_slugs);
  IF v_found <> array_length(v_slugs, 1) THEN
    RAISE EXCEPTION 'Expected 7 approved legal documents, found %', v_found;
  END IF;

  SELECT verified_by INTO v_approver
