...
const UnsubscribePage = () => {
  ...
  const confirm = async () => {
    ...
    if (result.reason === "already_used") return setStatus("already");
    if (result.reason === "expired") return setStatus("expired");
    if (result.reason === "invalid") return setStatus("invalid");
    setStatus("applied");
  };
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <Helmet>
        <title>Unsubscribe | Global Health Access Trust</title>
      ...
      <main className="flex-grow max-w-xl mx-auto px-6 py-20 w-full">
        ...
        {status === "ready" && (
          <>
            <p className="mb-6">
              Confirm that you would like to unsubscribe from this communication. This takes effect promptly and you do not need to sign in.
            </p>
        ...
        {status === "applied" && (
          <p>
            You have been unsubscribed. We will not use this address for this type of communication. Service messages related to your account or a donation may still be sent when legally required.
          </p>
        )}
        ...
        {status === "expired" && (
          <p>
            ...
            <Link to="/contact" className="underline">the secure contact form</Link>.
          </p>
        )}
        {status === "invalid" && (
          <p>
        ...
        {status === "error" && (
          <p>
            Something went wrong while checking this link. Please try again or use the{" "}
            <Link to="/contact" className="underline">secure contact form</Link>.
          </p>
        )}
      </main>
      <Footer />
